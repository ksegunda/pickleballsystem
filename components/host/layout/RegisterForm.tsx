...
interface RegisterFormProps {
  ...
  // "Sign in" link for an in-modal tab switch instead of a real
  // navigation. Standalone /register usage (no props) is unchanged.
  compact?:         boolean;
  onSwitchToLogin?: () => void;
}

export function RegisterForm({ compact = false, onSwitchToLogin }: RegisterFormProps = {}) {
  const router = useRouter();
  const [showPassword, setShowPassword] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  // Set once signup succeeds — swaps the whole form out for the OTP step,
  // in place, no navigation. Cleared by "use a different email" to go back.
