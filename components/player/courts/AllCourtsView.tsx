...
// Shared by the Courts grid — a lone court sits centered at the same width
// a grid cell would give it (instead of stretching bigger), so the layout
// doesn't visibly "jump" in scale as courts come and go.
...
// Active courts (a real, physical court with players on it right now) get
// the full pickleball-court graphic. A court with no match, or under
// maintenance, isn't "a court with people on it" — showing the graphic
// there would just be an empty illustration, so it stays a plain status
// row instead.
...
// Next Up sets aren't on a real court yet, so they stay a plain
// Team-A-vs-Team-B card (same language as the host's own Next Up cards)
// instead of the court graphic — that graphic is reserved for a court
// players are actually standing on.
